...
Create a heat map that displays the **number** of delayed flights
...
eek12/intro2de.ipynb)
  because the only requirement is that you use the `sflights.h5` file you have
created
...
When we have the `sflights.h5` file, the `data` DataFrame
  look as follows:
...
- Write a function named `pivot_table()` that takes a DataFrame.
  It should return a DataFrame that has the number of flights at ORD
  with departure delays greater than or equal to 15 minutes
